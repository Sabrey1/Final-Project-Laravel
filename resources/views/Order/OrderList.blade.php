            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg overflow-hidden">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="overflow-x-auto">
                        <form method="GET" action="{{ route('order.index') }}" class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <tbody>
                                 @forelse($order as $orders)
                                    <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900 transition">
                                        ...
                                        <td class="px-4 py-2 font-medium text-gray-800 dark:text-gray-100">{{ $orders->purchasedate }}</td>
                                        <td class="px-4 py-2">{{ $orders->refnumber ?? '-' }}</td>
                                        <td class="px-4 py-2">{{ $orders->vendorid ?? '-' }}</td>
                                        <td class="px-4 py-2">{{ $orders->employeeid ?? '-' }}</td>
                                        <td class="px-4 py-2 text-center">
                                            <div class="flex justify-center gap-2">
                                                <a href="{{route('order.show', $orders->id)}}" class="text-green-600 hover:text-green-800">
                                                    <i class="fa-solid fa-eye"></i>
                                                </a>
                                                <a href="{{route('order.edit', $orders->id)}}" class="text-blue-600 hover:text-blue-800">
                                                    <i class="fa-solid fa-pen-to-square"></i>
                                                </a>
                                                <form method="POST" action="{{ route('order.destroy', $orders->id) }}" onsubmit="return confirm('Are you sure?')" class="inline">
                                                    @csrf
                                                    @method('DELETE')
